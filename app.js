...
function highlightLastMove() {
    ...
    if (!h.length) return;
    var m = h[h.length - 1];
    $('#myBoard .square-' + m.from).addClass('hl-last');
    $('#myBoard .square-' + m.to).addClass('hl-last');
}
...
function selectSquare(square) {
    ...
    $('#myBoard .square-' + square).addClass('hl-select');
    var moves = game.moves({ square: square, verbose: true });
    moves.forEach(function(m) {
        $('#myBoard .square-' + m.to).addClass(m.captured ? 'hl-capture' : 'hl-move');
    });
}
...
function squareFromClassList(classStr) {
    var classes = (classStr || '').split(/\s+/);
    for (var i = 0; i < classes.length; i++) {
        if (/^square-[a-h][1-8]$/.test(classes[i])) return classes[i].slice(7);
    }
    return null;
}
...
function bindTapToMove() {
    $('#myBoard').on('click', '.square-55d63', function() {
        var sq = squareFromClassList(this.className);
